...
function LoadSelection()
{
	...
	sProtocol = sProtocol == null || sProtocol == '' || sProtocol == 'null' ? 'http://' : sProtocol;
...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
		case 'url' :
			...
			if(sUri.indexOf("<") != -1 || sUri.indexOf(">") != -1 || sUri.indexOf("&lt;") != -1 || sUri.indexOf("&gt;") != -1 || sUri.indexOf("&lt") != -1 || sUri.indexOf("&gt") != -1){
				alert( FCKLang.DlnLnkErrorMsg ) ;
				return false ;	
			}
			if ( sUri.length == 0 )
			{
			...
			//target = GetE('cmbLinkTarget').value;
	...
	var txtAttTitle = document.getElementById("txtAttTitle");
	if(txtAttTitle!="undefined"&&txtAttTitle.value.length<=0){
		alert("链接说明不能为空");
		return false ;
	}
