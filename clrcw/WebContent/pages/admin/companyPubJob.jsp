...
function del(s_id){
    ...
    if (d) {
        ...
        document.form1.action = "${ctx}/admin/aglientpublish.do?method=delPubJob";
        document.form1.submit();
    }
}
function endflag(s_id,validflag){
 if(validflag=='2'||validflag=='3'){
 alert("此条招聘信息已经结束招聘");
    ...
    if (d) {
        document.form1.id.value = s_id;
        document.form1.action = "${ctx}/admin/aglientpublish.do?method=savePubJobflag";
        document.form1.submit();
   }
    }
...
function postpone1(validtime,id){
    ...
    window.location.href="${ctx}/admin/aglientpublish.do?method=postponeJob&id="+id;
    alert("延期成功");
   }
   }
   }
...
if(address.length>100){
    ...
    if (welfare.length > 200) {
    	alert("福利待遇不得超出200字符");
        document.getElementById("welfare").focus();
        return false;
    }
    sage = document.getElementById("sage").value;
    eage = document.getElementById("eage").value;
    document.getElementById("age").value=sage+"--"+eage;
}
...
<%String compname=(String)request.getSession().getAttribute("compname"); %>
			<div id="content">
				...
				<DIV class="formdiv"
									...
									<logic:iterate id="info" name="job" type="model.PublishJobInfo">
										...
										<tr>
											...
											<td align="center"
												...
												<a class="tabBtn" href="${ctx}/admin/aglientpublish.do?method=updatePubJob&id=${info.id}">编辑</a>
												</c:if>
												<c:if test="${info.validflag=='3'||info.validflag=='2'}">
												<a class="tabBtn" href="${ctx}/admin/aglientpublish.do?method=showOneServicelog&jobid=${info.id}">查看</a>
												
												</c:if>
											
												<c:if test="${info.validflag=='1'}">
												
												<a href="#" class="tabBtn" onClick="endflag('${info.id}','${info.validflag}')">招聘结束</a>
												</c:if>
					...
					<form action="${ctx}/admin/aglientpublish.do?method=addPubJob"
							...
							<tr>
								...
								<td title="请选择职位">
									<select name="jobId" id="jobId" >
										...
										%>
										<option value="<%=bdJob.getId()%>"><%=bdJob.getJobName()%></option>
										<%
											}
										%>
									</select>
								...
								<td title="请输入年龄限制">
									<div><select name="stratage" id="sage"> 
										<% 
											for (int i = 16; i < 60; i++) { 
										%> 
										<option><%=i%></option> 
									...
									<select name="endage" id="eage"> 
										...
										%> 
									</select><input type="hidden" name="age" id="age"></div>
