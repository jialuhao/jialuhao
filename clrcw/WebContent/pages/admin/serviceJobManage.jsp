...
function checkservice(id){
 document.getElementById("jobid").value=id;
	    ...
	    document.forms[0].action="${ctx}/admin/findAllpublishJob.do?method=checkserviceJob";
...
function search(){
 	   document.forms[0].action="${ctx}/admin/findAllpublishJob.do?method=findAllserviceJob";
       document.forms[0].submit();
 }
...
function change(s_id){
    ...
    document.forms[0].action="${ctx}/admin/findAllpublishJob.do?method=nextfindservice";
	document.forms[0].submit();
}
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#cccccc">
		...
		<tr style="height: 20px">
			<td align="left" bgcolor="#FFFFFF" class="font5" style="padding-left: 30px;">
				...
				<td align="left" bgcolor="#FFFFFF" class="font5" style="padding-left: 30px;" colspan="2">
				招聘时间&nbsp;从：<input type="text" name="sTime" title="请选择开始时间" id="sTime" value="<%=session.getAttribute("sTime")!=null?session.getAttribute("sTime"):"" %>" onclick="calendar.show(this);" readonly="readonly" style="width: 80px;">&nbsp;至：
					<input type="text" name="eTime" title="请选择截止时间" id="eTime" value="<%=session.getAttribute("eTime")!=null?session.getAttribute("eTime"):"" %>" onclick="calendar.show(this);" readonly="readonly" style="width: 80px;">
...
<table width="98%" border="0" align="center" cellpadding="3"
	...
	<tr>
		...
		<td width="20%" align="center" bgcolor="#EEF5FD" class="font5">招聘单位</td>
		<td width="8%" align="center" bgcolor="#EEF5FD" class="font5">招聘职位</td>
	...
	<%
		...
		for (int i = 0; i < list.size(); i++) {
			PublishJobInfo userInfo = (PublishJobInfo) list.get(i);
			 ...
			 String gaing="0".equals(userInfo.getGrading())?"应届生":"往届生";
			 userInfo.getPkComp().getComp().getCompName();
         %>
	<tr>
		...
		<a	href="#" class="list"
			onClick="checkservice('<%=userInfo.getId()%>')">服务记录</a>
	...
	%>
	<tr>
		<td height="20" colspan="10" align="right" bgcolor="FEF9F9">总数：<%=(zshu+"  ") %>&nbsp;&nbsp;<%=pagenum+1%>/<%=pc+1 %>&nbsp;&nbsp;<a
	...
	%>
	<tr>
		<td height="20" colspan="10" align="right" bgcolor="FEF9F9">总数：<%=(zshu+"  ") %>&nbsp;&nbsp;<%=pagenum+1%>/<%=pc+1 %>&nbsp;&nbsp;</td>
	</tr>
	<%
		} else {
	%>
