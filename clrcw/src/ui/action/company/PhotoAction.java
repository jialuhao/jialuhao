...
public class PhotoAction extends BaseAction {
     ...
     */
	public ActionForward companyphoto(ActionMapping mapping, ActionForm form,
	...
	HttpServletRequest request, HttpServletResponse response) throws BaseException{
		try{
			...
			if(imageid ==null){
				imageid=(String)request.getAttribute("imageid");
			}
			response.reset();
            response.setContentType("image/jpeg");
			String id=request.getParameter("id");
			CompBespeak Lresume=new CompBespeak();
//			List eee = (List) super.getInviteManager().queryPersonResume(user);
//			List<?> list=super.getInviteManager().queryCompUserById(user);
			CompImage image=null;
//			if(list!=null && list.size()!=0){
//
//				Object[] o1=(Object[])list.get(0);
//				Lresume=(CompBespeak)o1[0];
//				String imageid=Lresume.getFkcompanyimage().toString();
//				
//				image=super.getInviteManager().findCompImageById(imageid);
//
//			}
			image=super.getInviteManager().findCompImageById(imageid);
			byte[] bPhoto=image.getCompImage();
